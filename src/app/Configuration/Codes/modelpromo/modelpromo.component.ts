...
import { ModelpromoService } from '../../../_services/modelpromo.service';

@Component({
  selector: 'app-modelpromo',
  templateUrl: './modelpromo.component.html',
  styleUrls: ['../../../../assets/css/configuration/configuration.css'],
  providers: [ModelpromoService]
})
export class ModelpromoComponent implements OnInit {
  ...
  editerrormsg: string = "";
  pgTitle: string = "Model Promotion";
  minDate: Date;
  pager: any = {};
  pagedItems: any[];
  selectpage: number = 1;
  ...
  constructor(private fb: FormBuilder,
    ...
    private http: Http,
    private ModelpromoService: ModelpromoService, private pagerService: PagerService) {
    this.sortby = "Code";
    this.sortorder = "asc";
    this.getData();
  }
  ...
  dataSource: MatTableDataSource<ModelMPData>;
  ...
  getData() {
    ...
    this.ModelpromoService.getMPData()
      .subscribe(res => {
        this.codesList = res;
      ...
      err => {
        this.errorMsg = "Model Promotion codes not found.";
  ...
  callMPCodes() {
    ...
    this.code = "";
    this.description = "";
    this.pgTitle = "Add Model Promotion Code";
  }
  addMPcodeback() {
    this.addFlag = false;
  ...
  saveMPcode(value) {
    ...
    if (this.codeForm.valid) {
      ...
      this.ModelpromoService.submitMP(body)
        .subscribe(res => {
          this.addFlag = false;
          this.codesList = "";
          this.displayFlag = true;
  ...
  editMPcodes(id, code, desc, startDate, endDate) {
    this.editFlag = true;
    this.displayFlag = false;
    this.editcode = code;
    ...
    this.pgTitle = "Edit Model Promotion Code";
  }
  updateMPeditcode(code, value) {
    this.editerrormsg = "";
    if (this.codeeditForm.valid) {
      ...
      this.ModelpromoService.updateMP(this.id, body)
        .subscribe(res => {
          this.editFlag = false;
          this.codesList = "";
          this.displayFlag = true;
