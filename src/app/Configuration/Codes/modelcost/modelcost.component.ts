...
import { ModelcostService } from '../../../_services/modelcost.service';

@Component({
  selector: 'app-modelcost',
  templateUrl: './modelcost.component.html',
  styleUrls: ['../../../../assets/css/configuration/configuration.css'],
  providers: [ModelcostService]
})
export class ModelcostComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    private http: Http,
    private ModelcostService: ModelcostService, private pagerService: PagerService) {
    this.sortby = "Code";
    this.sortorder = "asc";
    this.getData();
  }
  ...
  dataSource: MatTableDataSource<ModelcostData>;
  ...
  getData() {
    ...
    this.ModelcostService.getMCData()
      .subscribe(res => {
        this.codesList = res;
      ...
      err => {
        this.errorMsg = "Model Cost codes not found.";
  ...
  callMCCodes() {
    ...
    this.code = "";
    this.description = "";
    this.pgTitle = "Add Model Cost Code";
  }
  addMCcodeback() {
    this.addFlag = false;
  ...
  saveMCcode(value) {
    ...
    if (this.codeForm.valid) {
      ...
      this.ModelcostService.submitMC(body)
        .subscribe(res => {
          this.addFlag = false;
          this.codesList = "";
          this.displayFlag = true;
  ...
  editMCcodes(id, code, desc, startDate, endDate, isSystem) {
    this.editFlag = true;
    this.displayFlag = false;
    this.editcode = code;
    ...
    this.pgTitle = "Edit Model Cost Code";
  }

  updateMCeditcode(code, value) {
    this.editerrormsg = "";
    if (this.codeeditForm.valid) {
      ...
      this.ModelcostService.updateMC(this.id, body)
        .subscribe(res => {
          this.editFlag = false;
          this.codesList = "";
          this.displayFlag = true;
