...
import { ARtransactionService } from '../../../_services/artransaction.service';

@Component({
  selector: 'app-ARtransaction',
  templateUrl: './artransaction.component.html',
  styleUrls: ['../../../../assets/css/configuration/configuration.css'],
  providers: [ARtransactionService]
})
export class ARtransactionComponent implements OnInit {
  ...
  ETchecked: boolean = true;

  constructor(private fb: FormBuilder,
    private router: Router,
    private http: Http,
    private ARtransactionService: ARtransactionService, private pagerService: PagerService) {
    this.sortby = "Code";
    this.sortorder = "asc";
    this.getData();
  }
  ...
  dataSource: MatTableDataSource<ARtransactionData>;
  ...
  getData() {
    ...
    this.ARtransactionService.getARTData()
      .subscribe(res => {
        this.codesList = res;
      ...
      err => {
        this.errorMsg = "A/R Trasaction codes not found.";
  ...
  callARTCodes() {
    ...
    this.code = "";
    this.description = "";
    this.pgTitle = "Add A/R Transaction Code";
  }
  addARTcodeback() {
    this.addFlag = false;
  ...
  saveARTcode(value) {
    ...
    if (this.codeForm.valid) {
      ...
      this.ARtransactionService.submitART(body)
        .subscribe(res => {
          this.addFlag = false;
          this.codesList = "";
          this.displayFlag = true;
          this.getData();
          this.pgTitle = "A/R Transaction";
          this.setPage(1);
        },
        err => {
          err = err
  ...
  editARTcodes(id, code, desc, startDate, endDate, isSystem) {
    this.editFlag = true;
    this.displayFlag = false;
    this.editcode = code;
    ...
    this.pgTitle = "Edit A/R Transaction Code";
  }
  updateARTeditcode(code, value) {
    this.editerrormsg = "";
    if (this.codeeditForm.valid) {
      ...
      this.ARtransactionService.updateART(this.id, body)
        .subscribe(res => {
          this.editFlag = false;
          this.codesList = "";
          this.displayFlag = true;
...
export interface ARtransactionData {
  Id: any;
  Code: any;
  StartDate: any;
  EndDate: any;
